...
local bin genital_HPV16 genital_HPV18 genital_HPV16_18 genital_HPV6_11 genital_vactypes genital_nonovalent genital_HRtypes 
foreach b in `bin'{
recode `b' 1=2 
recode `b' 0=1
}
...
putexcel A1=("risk factor")  B1=("level") C1=("seroneg-n (%)") D1=("seropos n(%)") E1=("OR") F1=("95%CI") G1=("aOR") H1=("a95%CI") using sero_DNAgen, replace
	...
	foreach y of varlist `sero'{
		foreach i of varlist `bin'{
				levelsof `i', local(levels)
				 ...
				 quietly logistic `y' i.`i' if adequatepcr2==1|adequatepcr3==1|adequatepcr4==1
				matrix b=r(table)
					...
					quietly logistic `y' i.`i' age i.gp_nummen_life if adequatepcr2==1|adequatepcr3==1|adequatepcr4==1
				matrix ab=r(table)
				...
				putexcel A`row'=("`i'") B`row'=(`l') C`row'=("`n_neg' (`per_neg')") D`row'=("`n_pos' (`per_pos')")  E`row'=(`OR') F`row'=("(`lc'-`uc')") G`row'=(`aOR') H`row'=("(`alc'-`auc')") using sero_DNAgen, modify
				loc row = `row' + 1
						}
						}
						}
